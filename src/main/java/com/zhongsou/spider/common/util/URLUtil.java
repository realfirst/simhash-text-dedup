...
import com.zhongsou.spider.common.util.domain.DomainSuffix;
import com.zhongsou.spider.common.util.domain.DomainSuffixes;
...
public class URLUtil {
	...
	public static URL resolveURL(URL base, String target)
		 ...
		 * that are embedded into the base url - move them to target // so URL
		 * class constructs the new url class properly if
		 * (base.toString().indexOf(';') > 0) return fixEmbeddedParams(base,
		 * target);
		 */
	...
	public static String hostReverse(String host) {
		String t[] = host.split("\\.");
		String reverseHost = "";
	...
	public static void main(String[] args) {
		...
		String purl = "http://www.sina.com/dfa/adfa?a=b&amp;c=d";

		/*
		 * if(args.length!=1) { System.err.println("Usage : URLUtil <url>");
		 * return ; }
		 */

		/*
		 * String url = args[0]; try {
		 * System.out.println(URLUtil.getDomainName(new URL(url))); } catch
		 * (MalformedURLException ex) { ex.printStackTrace(); }
		 */
		String host = "adaf.ccdad.da.com.cn";
		System.out.println("domain=" + URLUtil.getDomainByHost(host) + "\t"
				+ URLUtil.hostReverse(host));
	}
}
