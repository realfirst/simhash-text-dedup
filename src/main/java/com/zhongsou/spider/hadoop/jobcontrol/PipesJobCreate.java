...
public class PipesJobCreate extends Configured implements Tool {
	protected static final Log LOG = LogFactory.getLog(PipesJobCreate.class);

	public PipesJobCreate() {
		this(new Configuration());
	}

	public PipesJobCreate(Configuration conf) {
		setConf(conf);
	}
	...
	public RunningJob runJob(String args[]) throws Exception {
		JobConf conf = this.parseJobConf(args);
		setupPipesJob(conf);
		return JobClient.runJob(conf);
	}

	public RunningJob submitJob(String args[]) throws Exception {
		JobConf conf = this.parseJobConf(args);
		setupPipesJob(conf);
		JobClient jobClient = new JobClient(conf);
		return jobClient.submitJobInternal(conf);
		// jobClient.su
	}
	...
	private static void setupPipesJob(JobConf conf) throws IOException {
		...
		if (!getIsJavaMapper(conf)) {
			conf.set("mapred.map.runner.class", "org.apache.hadoop.mapred.pipes.PipesMapRunner");
			...
			conf.set("mapred.partitioner.class", "org.apache.hadoop.mapred.pipes.PipesPartitioner");
		}
		if (!getIsJavaReducer(conf)) {
			// conf.setReducerClass(PipesReducer.class);
			conf.set("mapred.reducer.class", "org.apache.hadoop.mapred.pipes.PipesReducer");
		...
		if (!getIsJavaRecordReader(conf) && !getIsJavaMapper(conf)) {
			...
			conf.set("mapred.input.format.class", "org.apache.hadoop.mapred.pipes.PipesNonJavaInputFormat");
		}
	...
	public JobConf parseJobConf(String args[]) throws Exception {
	...
	public static void main(String[] args) throws Exception {
		// args="-libjars  /home/dape/workspace_java/spider_common/target/spider_common-0.0.1-SNAPSHOT.jar -Dmapred.reduce.tasks=6 -Dmapred.job.name=score_url -Dhbase.mapreduce.inputtables=webDB,selectHostDB -Dscore_level=4 -Dread_black_list=true -files=/home/dape/workspace_java/blackhost.txt,/home/dape/workspace_java/blackdomain.txt -Dmapred.reduce.tasks=15 -conf /home/dape/workspace_java/spider_c++_common/conf/pipes_score_conf.xml  -inputformat com.zhongsou.spider.hbase.MultiTableInputFormat -output score_output -writer org.apache.hadoop.mapred.SequenceFileOutputFormat  -program bin/score_url".split(" {1,}");
		int exitCode = new PipesJobCreate().run(args);
